...
merge(){
        ...
        cat $(git rev-parse --show-toplevel)/.git/config | grep -E "\[branch" | sed -e 's/\"\(.*\)\"/\1/' | sed 's/[][]//g' | awk -F" " '{print $2}' | grep -v "$cur_branch" > tmp
...
pullreq(){
	...
	read -p "Pull Request Title: " title
...
ignore(){
	...
	read ign -p "Input extensions to ignore (as comma-separated list): " ign
